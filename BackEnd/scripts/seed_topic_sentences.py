...
SEED: list[dict] = [
    ...
    {
        "topic": "drop_out",
        "note": "'학교 그만두려면 어떻게 해?'가 휴학 문서를 잡던 문제",
        # 학생은 '자퇴'라는 공식어를 잘 쓰지 않는다. '자퇴하려면 어떻게 해?'는 0.984로
        # 확실히 잡히는데, 같은 뜻을 일상어로 물으면 leave(휴학)로 샜다(0.762 vs 0.747).
        # 두 주제가 '학업 중단'이라는 점에서 가까워 구어체에서는 구분이 안 됐다.
        # 자퇴를 '학교를 그만둔다'는 일상 표현으로 가르치는 문장이 없어서 생긴 공백이다.
        #
        # 적용 전 시뮬레이션(변경 전 → 후):
        #   학교 그만두려면 어떻게 해?   leave 0.762 → drop_out 0.852
        #   학교 그만두고 싶어          leave 0.659 → drop_out 0.788
        #   자퇴하려면 어떻게 해?        drop_out 0.984 → 변화 없음
        #   휴학 어떻게 신청해? / 복학 절차 / 재입학  변화 없음
        "add": [
            "학교를 그만두려면 어떻게 해야 하나요?",
            "학교를 계속 다니지 않으려면 어떤 절차가 필요한가요?",
            "학업을 완전히 중단하고 싶습니다.",
        ],
    },
    {
        "topic": "leave",
        "note": "'군대 가는데 학교는 어떻게 해?'가 학군단 안내를 답하던 문제",
        # 이 건은 '못 찾음'이 아니라 오답이라 더 위험했다. 입대휴학을 물었는데 rotc로
        # 라우팅돼(0.677 vs leave 0.646) 학군사관 후보생 혜택 안내가 자신 있게 나갔다.
        # '군대'라는 낱말을 가진 문장이 rotc에만 있어서, 군 관련 질문이 전부 그쪽으로 끌렸다.
        #
        # 입대휴학 규정은 leave에 있다(입영통지서 첨부, 입영일로부터 1개월 이내 등).
        # '군 복무 때문에 학업을 멈춘다'는 관점의 문장을 leave에 세워 rotc 흡인을 끊는다.
        #
        # 적용 전 시뮬레이션(변경 전 → 후):
        #   군대 가는데 학교는 어떻게 해?  rotc 0.677 → leave 0.789
        #   입대하는데 어떻게 해야 돼?     return_to_school 0.686 → leave 0.786
        #   학군단 지원 자격이 뭐야        rotc 0.918 → 변화 없음
        #   군대 때문에 휴학하려면?        leave 0.858 → 변화 없음
        "add": [
            "군대에 가는데 학교는 어떻게 해야 하나요?",
            "입대하게 되면 어떤 절차를 밟아야 하나요?",
            "군 복무 때문에 학업을 중단해야 합니다.",
        ],
    },
]
...
CHECK = [
    ...
    # 공식어(자퇴·휴학) 없이 일상어로 물었을 때 — 앞의 둘은 자퇴, 셋째는 입대휴학이어야 한다
    "학교 그만두려면 어떻게 해?", "학교 그만두고 싶어", "군대 가는데 학교는 어떻게 해?",
]
