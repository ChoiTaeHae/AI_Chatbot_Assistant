...
DEFAULT_SEARCH_SYNONYMS: dict[str, list[str]] = {
    ...
    # 학생은 '전과'라는 공식어를 잘 안 쓴다. 토픽 라우팅은 major_change로 제대로 가는데
    # 구어체 그대로 검색되면 리랭커가 전부 0점을 줘 청크 0개로 끝났다(실측).
    # '전과'를 덧붙이면 3청크가 살아난다. 두 항목 모두 0건 → 3건으로 구제되는 것만 넣었다.
    # ('전공 바꾸'는 원문만으로 이미 3청크라 불필요하고, '부전공 바꾸고 싶어'에 끼어들어 제외)
    "과 옮기": ["전과"],      # 실측 '과 옮기고 싶은데 어떻게 해?' 0건 → 3건
    "과 바꾸": ["전과"],      # 실측 '과 바꾸고 싶은데 어떻게 해?' 0건 → 3건
    # 같은 이유로 입대휴학도 구어체에서 0건이 된다. 단 '입대'로 등록하면 안 된다 —
    # 부분 문자열이라 '편입대상자'·'편입대학'·'신입대비반'에 걸린다(실측). 어미까지 붙여
    # 좁히면 오탐이 사라지고, 두 형태로 '입대하는데/입대하면/입대해야'를 모두 잡는다.
    "입대하": ["휴학"],       # 실측 '입대하는데 어떻게 해야 돼?' 0건 → 2건(1등 0.786)
    "입대해": ["휴학"],
}
...
async def answer_rag_general_question_with_metadata(
    ...
    if not context:
        ...
        if fallback_files:
            ...
            return (
                "질문하신 내용은 코파일럿이 글로 정리해 둔 자료에는 없지만, 관련 안내 파일이 준비되어 있어요.\n\n"
