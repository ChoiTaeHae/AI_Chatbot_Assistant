...
export default function ScholarshipManager() {
  ...
  const inputCls = 'w-full text-sm text-[#05263d] border border-slate-200 rounded-lg bg-white outline-none focus:border-[#005956]'

  // ─────────────────────────────── 목록 뷰 ───────────────────────────────
  if (mode === 'list') {
    return (
      <div className="flex-1 bg-white rounded-2xl shadow-sm border border-slate-100 flex flex-col" style={{ padding: '32px', gap: '18px' }}>
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-base font-black text-[#05263d]">장학금·근로 관리</h2>
            <p className="text-xs text-slate-400 mt-0.5">'장학금·근로 둘러보기'에 노출되는 항목을 추가·수정하고 파일을 연결합니다</p>
          </div>
          <button onClick={openAdd} className="flex items-center bg-[#005956] text-white text-sm font-bold hover:bg-[#004a47] transition rounded-xl" style={{ gap: '6px', padding: '9px 16px' }}>
        ...
        {loading ? (
          <p className="text-center text-slate-400 text-sm" style={{ padding: '40px' }}>불러오는 중…</p>
        ) : list.length === 0 ? (
          <div className="flex flex-col items-center justify-center text-slate-300" style={{ padding: '50px 0', gap: '8px' }}>
          ...
          <div className="flex-1 overflow-y-auto flex flex-col" style={{ gap: '18px' }}>
            {['장학금', '근로'].map((k) => (grouped[k] || []).length > 0 && (
              <div key={k}>
                <p className="text-sm font-bold" style={{ marginBottom: '8px', color: TEAL }}>{k === '근로' ? '💼' : '🎓'} {k} <span className="text-slate-400 font-normal">({grouped[k].length})</span></p>
                <div className="border border-slate-200 rounded-xl overflow-hidden">
                  {grouped[k].map((s) => (
                    <div key={s.id} className="flex items-center gap-3 border-b border-slate-100 last:border-b-0 hover:bg-slate-50 transition" style={{ padding: '11px 14px' }}>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="text-[11px] font-semibold text-white rounded-full shrink-0" style={{ padding: '1px 8px', background: s.scope === '교외' ? '#0ea5a0' : '#94a3b8' }}>{s.scope}</span>
                          <span className="font-semibold text-slate-800 text-sm">{s.name}</span>
                          {s.category && <span className="text-[11px] text-slate-500 bg-slate-100 rounded-full" style={{ padding: '1px 8px' }}>{s.category}</span>}
                        ...
                        <div className="flex items-center gap-2 text-[11px] text-slate-400" style={{ marginTop: '2px' }}>
                          {s.period && <span>🗓 {s.period}</span>}
                          <span>📎 파일 {s.files?.length || 0}</span>
                        </div>
                      </div>
                      <button onClick={() => openEdit(s)} className="text-xs font-semibold text-[#005956] hover:underline shrink-0">수정</button>
  ...
  // ─────────────────────────────── 추가/수정 폼 ───────────────────────────────
  return (
    <div className="flex-1 bg-white rounded-2xl shadow-sm border border-slate-100 flex flex-col overflow-y-auto" style={{ padding: '32px', gap: '16px' }}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button onClick={backToList} className="text-slate-400 hover:text-slate-700" aria-label="목록으로">
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" /></svg>
          </button>
          <h2 className="text-base font-black text-[#05263d]">{editingId ? `${form.kind} 수정` : `${form.kind} 추가`}</h2>
        </div>
        <button onClick={save} disabled={saving} className="bg-[#005956] text-white text-sm font-bold hover:bg-[#004a47] transition disabled:opacity-50 rounded-xl" style={{ padding: '9px 18px' }}>
          {saving ? '저장 중…' : '저장'}
        </button>
      </div>
      ...
      <div className="grid grid-cols-2" style={{ gap: '14px' }}>
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs font-bold text-slate-500">종류 *</span>
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs font-bold text-slate-500">구분 *</span>
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs font-bold text-slate-500">카테고리 (그룹)</span>
        ...
        <label className="col-span-2 flex flex-col gap-1">
          <span className="text-xs font-bold text-slate-500">지원 조건 (한 줄)</span>
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs font-bold text-slate-500">신청 기간 (화면 표시)</span>
        ...
        <label className="flex flex-col gap-1">
          <span className="text-xs font-bold text-slate-500">마감 일시 <span className="text-slate-400 font-normal">(지나면 자동 '기간마감', 상시는 비움)</span></span>
        ...
        <label className="col-span-2 flex flex-col gap-1">
          <span className="text-xs font-bold text-slate-500">외부 공고 링크</span>
      ...
      <div className="border-t border-slate-100" style={{ paddingTop: '16px' }}>
        <div className="flex items-center justify-between" style={{ marginBottom: '4px' }}>
          <p className="text-sm font-bold text-[#05263d]">파일 첨부</p>
          {editingId && (
            <>
              <input ref={fileInputRef} type="file" className="hidden"
                accept=".pdf,.docx,.pptx,.xlsx,.hwp,.hwpx,.txt,.md,.jpg,.jpeg,.png" onChange={handleUpload} />
              <button onClick={() => fileInputRef.current?.click()} disabled={uploading}
                className="flex items-center gap-1 text-xs font-bold text-white bg-[#005956] hover:bg-[#004a47] transition disabled:opacity-50 rounded-lg" style={{ padding: '6px 12px' }}>
        ...
        {!editingId ? (
          <p className="text-xs text-slate-400">먼저 <b>저장</b>하면 이 장학금에 파일을 업로드·연결할 수 있어요.</p>
        ) : (
          <>
            <p className="text-xs text-slate-400" style={{ marginBottom: '10px' }}>파일을 <b>업로드</b>하면 이 항목에 바로 연결돼요(현재 탭 종류로 저장). 기존 파일은 체크해서 연결하고, 대표 1개를 지정하세요.</p>
              ...
              {[['scholarship', '장학금 파일'], ['work_study', '근로 파일']].map(([t, label]) => {
                ...
                return (
                  <button key={t} onClick={() => setFileTab(t)} className="text-xs font-semibold transition"
                    style={{ padding: '5px 12px', borderRadius: '999px', background: active ? TEAL : '#f1f5f9', color: active ? '#fff' : '#64748b' }}>
                    {label} ({fileCounts[t] || 0})
                  </button>
                )
              })}
            </div>
            <div className="flex items-center gap-2 rounded-lg border border-slate-200" style={{ padding: '6px 10px', marginBottom: '10px' }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2"><circle cx="11" cy="11" r="7" /><path d="m21 21-4.3-4.3" strokeLinecap="round" /></svg>
            ...
            <div className="border border-slate-200 rounded-xl overflow-hidden" style={{ maxHeight: '320px', overflowY: 'auto' }}>
              {shownFiles.length === 0 ? (
                <p className="text-center text-slate-400 text-xs" style={{ padding: '20px' }}>이 탭에 파일이 없어요. 위 '파일 업로드'로 올리세요.</p>
              ) : shownFiles.map((f) => {
                const linkedHere = f.scholarship_id === editingId
                const linkedOther = f.scholarship_id && !linkedHere
                return (
                  <div key={f.id} className="flex items-center gap-2 border-b border-slate-50 last:border-b-0" style={{ padding: '8px 12px', background: linkedHere ? '#f0f9f8' : 'transparent' }}>
                    <input type="checkbox" checked={linkedHere} onChange={() => toggleFile(f)} style={{ accentColor: TEAL }} />
                    <span className="flex-1 min-w-0 truncate text-xs text-slate-700" title={f.name}>{f.name}</span>
                    ...
                    <button onClick={() => handleDeleteFile(f)} className="text-slate-300 hover:text-red-500 transition shrink-0" title="파일 삭제" aria-label="파일 삭제">
