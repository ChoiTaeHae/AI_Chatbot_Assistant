...
function highlight(text, q) {
  ...
  return (
    <>
      {text.slice(0, i)}
      <span style={{ background: '#E1F5EE', color: TEAL, borderRadius: '3px', padding: '0 2px' }}>{text.slice(i, i + q.length)}</span>
...
export default function ScholarshipModal({ lang = 'ko', onClose }) {
  ...
  return (
    <div
      ...
      style={{ background: 'rgba(15,23,42,.5)', paddingLeft: isDesktop ? '288px' : '16px', paddingRight: '16px' }}
      onClick={onClose}
    >
      <div
      ...
      >
        {/* 헤더 */}
        <div className="flex items-center gap-3 border-b border-slate-100 shrink-0" style={{ padding: '16px 18px' }}>
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-slate-800" style={{ fontSize: '15px' }}>{mt.title}</p>
            <p className="text-slate-400" style={{ fontSize: '12px' }}>{mt.subtitle}</p>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-700 text-lg" aria-label={mt.close}>✕</button>
        </div>

        {/* 상단 전환: 장학금 / 근로 */}
        <div className="flex items-center gap-2 border-b border-slate-100 shrink-0" style={{ padding: '12px 18px' }}>
          {[['장학금', '🎓'], ['근로', '💼']].map(([k, icon]) => {
            const active = kind === k
            return (
              <button
        ...
        {/* 탭 + 검색 */}
        <div className="border-b border-slate-100 shrink-0" style={{ padding: '12px 18px' }}>
          ...
          <div className="flex items-center gap-2 rounded-full border border-slate-200" style={{ padding: '7px 12px' }}>
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2" aria-hidden="true"><circle cx="11" cy="11" r="7" /><path d="m21 21-4.3-4.3" strokeLinecap="round" /></svg>
            <input
              value={query}
            ...
            />
            {query && (
              <button onClick={() => onSearch('')} className="text-slate-300 hover:text-slate-500" aria-label={mt.clearSearch}>✕</button>
            )}
          </div>
          <label className="flex items-center gap-1.5 cursor-pointer select-none w-fit" style={{ marginTop: '9px', fontSize: '12px', color: '#64748b' }}>
        ...
        <div style={{ padding: '12px 18px 18px', overflowY: 'auto' }}>
          {loading && <p className="text-center text-slate-400" style={{ fontSize: '13px', padding: '30px' }}>{mt.loading}</p>}
          {error && !loading && <p className="text-center text-red-400" style={{ fontSize: '13px', padding: '30px' }}>{error}</p>}
          {!loading && !error && visibleCount === 0 && (
            <p className="text-center text-slate-400" style={{ fontSize: '13px', padding: '30px' }}>
          ...
          {!loading && !error && visibleGroups.map((g) => {
            ...
            return (
              <div key={g.category} className="rounded-xl overflow-hidden" style={{ marginBottom: '14px', border: '1px solid #cbd5e1' }}>
                <button
                  onClick={() => setOpenCats((p) => ({ ...p, [g.category]: !p[g.category] }))}
                  className="flex items-center gap-2 w-full transition hover:brightness-95"
                  style={{ padding: '11px 14px', background: '#e1f5ee' }}
                >
                ...
                {open && (
                  <div style={{ padding: '4px 14px 10px' }}>
                    {g.items.map((it) => {
                      ...
                      return (
                      <div key={it.id} className="border-b border-[#9fabbb] last:border-b-0" style={{ padding: '14px 4px' }}>
                        <div className="flex items-center flex-wrap" style={{ columnGap: '8px', rowGap: '4px' }}>
                          <span className="font-medium text-slate-800" style={{ fontSize: '13px' }}>{highlight(it.name, query)}</span>
                          {it.amount && <span className="font-semibold rounded-full shrink-0" style={{ fontSize: '11px', padding: '2px 9px', background: '#e1f5ee', color: TEAL }}>{it.amount}</span>}
                        </div>
                        {/* 기간 + 마감 */}
                        {(it.period || it.expired) && (
                          <div className="flex items-center flex-wrap gap-2" style={{ marginTop: '6px' }}>
                            {it.period && (
                              <span className="rounded-full inline-flex items-center gap-1" style={{ fontSize: '11px', padding: '2px 9px', background: it.expired ? '#fee2e2' : '#fef3c7', color: it.expired ? '#b91c1c' : '#92400e' }}>
                                🗓 {it.period}
                              </span>
                            )}
                            {it.expired && (
                              <span className="rounded-full font-semibold" style={{ fontSize: '11px', padding: '1px 8px', background: '#fee2e2', color: '#b91c1c' }}>{mt.expired}</span>
                            )}
                          </div>
                        )}
                        ...
                        {/* 조건 */}
                        {it.eligibility && (
                          <p className="text-slate-600" style={{ fontSize: '12px', marginTop: '7px' }}>
                            <span className="text-slate-400">{mt.condition} : </span>{highlight(it.eligibility, query)}
                          </p>
                        )}

                        {/* 안내 문구 */}
                        {files.length > 0 && (
                          <p className="text-slate-400" style={{ fontSize: '11px', marginTop: '3px' }}>{mt.detailNote}</p>
                        )}
                          ...
                          <div className="flex items-center flex-wrap gap-2" style={{ marginTop: '10px' }}>
                            {files.length > 0 && (
                              <button
                                ...
                                className="inline-flex items-center gap-1 rounded-full hover:bg-[#e4f4f3] transition"
                        ...
                        {filesOpen && files.length > 0 && (
                          <div className="flex flex-col rounded-lg" style={{ gap: '2px', marginTop: '8px', padding: '8px 10px', background: '#f8fafc' }}>
                            {files.map((f) => (
                              <button
                                key={f.name}
                                onClick={() => downloadFileWithAuth(f.topic, f.name)}
                                className="inline-flex items-center gap-1.5 text-left hover:text-[#005956] transition"
                                style={{ fontSize: '12px', color: '#475569', padding: '2px 0' }}
                                title={f.name}
                              >
