      ...
      >
        ...
        <div className="flex items-center gap-3 border-b border-slate-100 shrink-0" style={{ padding: '16px 18px' }}>
          ...
          <div className="flex-1 min-w-0">
            <p className="font-bold text-slate-800" style={{ fontSize: '15px' }}>장학금·근로 둘러보기</p>
            <p className="text-slate-400" style={{ fontSize: '12px' }}>우송대학교 장학·근로 안내</p>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-700 text-lg" aria-label="닫기">✕</button>
        </div>
        ...
        <div className="border-b border-slate-100 shrink-0" style={{ padding: '12px 18px' }}>
          ...
          <div className="flex items-center gap-2 rounded-full border border-slate-200" style={{ padding: '7px 12px' }}>
            ...
            />
            {query && (
              <button onClick={() => onSearch('')} className="text-slate-300 hover:text-slate-500" aria-label="검색어 지우기">✕</button>
            )}
          </div>
        ...
        <div style={{ padding: '12px 18px 18px', overflowY: 'auto' }}>
          {loading && <p className="text-center text-slate-400" style={{ fontSize: '13px', padding: '30px' }}>불러오는 중…</p>}
                      ...
                      <div key={it.id} className="border-b border-[#9fabbb] last:border-b-0" style={{ padding: '14px 4px' }}>
                        ...
                        {(it.period || it.expired) && (
                          <div className="flex items-center flex-wrap gap-2" style={{ marginTop: '6px' }}>
                            ...
                            {it.expired && (
                              <span className="rounded-full font-semibold" style={{ fontSize: '11px', padding: '1px 8px', background: '#fee2e2', color: '#b91c1c' }}>기간마감</span>
                            )}
                          </div>
                        )}
                        ...
                        {it.eligibility && (
                          <p className="text-slate-600" style={{ fontSize: '12px', marginTop: '7px' }}>
                            <span className="text-slate-400">조건 : </span>{highlight(it.eligibility, query)}
                          </p>
                        )}

                        {/* 안내 문구 */}
                        {files.length > 0 && (
                          <p className="text-slate-400" style={{ fontSize: '11px', marginTop: '3px' }}>세부 내용은 첨부된 공고문을 확인해 주세요</p>
                        )}
